...
* `TauCeti.Comodule.matrixCoefficientSubalgebraWithAntipode`: the algebra generated by
...
For a finite projective comodule, `matrixCoefficientSubalgebra_prod_dual` identifies this
intrinsically defined algebra with the coefficient algebra of `M × Mᵛ`. -/
def matrixCoefficientSubalgebraWithAntipode : Subalgebra R H :=
  matrixCoefficientSubalgebra (R := R) (C := H) (M := M) ⊔
    (matrixCoefficientSubalgebra (R := R) (C := H) (M := M)).map
      (HopfAlgebra.antipodeAlgHom R H)

/-- Every matrix coefficient belongs to the algebra generated by the coefficients and their
antipode images. -/
@[simp]
theorem matrixCoefficient_mem_matrixCoefficientSubalgebraWithAntipode
    (φ : M →ₗ[R] R) (m : M) :
    matrixCoefficient (R := R) (C := H) φ m ∈
      matrixCoefficientSubalgebraWithAntipode (R := R) (H := H) (M := M) := by
  unfold matrixCoefficientSubalgebraWithAntipode
  exact (le_sup_left : matrixCoefficientSubalgebra (R := R) (C := H) (M := M) ≤ _)
    (matrixCoefficient_mem_subalgebra (R := R) (C := H) φ m)

/-- The antipode of every matrix coefficient belongs to the algebra generated by the
coefficients and their antipode images. -/
@[simp]
theorem antipode_matrixCoefficient_mem_matrixCoefficientSubalgebraWithAntipode
    (φ : M →ₗ[R] R) (m : M) :
    HopfAlgebra.antipode R (matrixCoefficient (R := R) (C := H) φ m) ∈
      matrixCoefficientSubalgebraWithAntipode (R := R) (H := H) (M := M) := by
  unfold matrixCoefficientSubalgebraWithAntipode
  apply (le_sup_right :
    (matrixCoefficientSubalgebra (R := R) (C := H) (M := M)).map
      (HopfAlgebra.antipodeAlgHom R H) ≤ _)
  exact ⟨matrixCoefficient (R := R) (C := H) φ m,
    matrixCoefficient_mem_subalgebra (R := R) (C := H) φ m, rfl⟩

/-- The coefficient-and-antipode algebra is contained in a subalgebra exactly when that
subalgebra contains every coefficient and every antipode image of a coefficient. -/
theorem matrixCoefficientSubalgebraWithAntipode_le_iff {S : Subalgebra R H} :
    matrixCoefficientSubalgebraWithAntipode (R := R) (H := H) (M := M) ≤ S ↔
      (∀ (φ : M →ₗ[R] R) (m : M), matrixCoefficient (R := R) (C := H) φ m ∈ S) ∧
      (∀ (φ : M →ₗ[R] R) (m : M),
        HopfAlgebra.antipode R (matrixCoefficient (R := R) (C := H) φ m) ∈ S) := by
  constructor
  · intro h
    exact ⟨fun φ m ↦ h (matrixCoefficient_mem_matrixCoefficientSubalgebraWithAntipode φ m),
      fun φ m ↦
        h (antipode_matrixCoefficient_mem_matrixCoefficientSubalgebraWithAntipode φ m)⟩
  · rintro ⟨h, hS⟩
    unfold matrixCoefficientSubalgebraWithAntipode
    apply sup_le
    · exact matrixCoefficientSubalgebra_le h
    · rw [matrixCoefficientSubalgebra_def, AlgHom.map_adjoin]
      apply Algebra.adjoin_le
      rintro _ ⟨c, hc, rfl⟩
      obtain ⟨φ, m, rfl⟩ :=
        (mem_matrixCoefficientSet_iff (R := R) (C := H) (M := M) c).mp hc
      rw [HopfAlgebra.antipodeAlgHom_apply]
      exact hS φ m

/-- The algebra generated by coefficients and antipode images is the algebra adjoined by the
union of the corresponding two coefficient sets. -/
theorem matrixCoefficientSubalgebraWithAntipode_eq_adjoin :
    matrixCoefficientSubalgebraWithAntipode (R := R) (H := H) (M := M) =
      Algebra.adjoin R
        (matrixCoefficientSet (R := R) (C := H) (M := M) ∪
          HopfAlgebra.antipode R '' matrixCoefficientSet (R := R) (C := H) (M := M)) := by
  unfold matrixCoefficientSubalgebraWithAntipode
  rw [matrixCoefficientSubalgebra_def, AlgHom.map_adjoin, Algebra.adjoin_union]
  congr 2
...
theorem coefficientMatrix_mem_matrixCoefficientSubalgebraWithAntipode
    {x : Type*} (b : Module.Basis x R M) (i j : x) :
    coefficientMatrix (C := H) b i j ∈
      matrixCoefficientSubalgebraWithAntipode (R := R) (H := H) (M := M) := by
  rw [coefficientMatrix_apply]
  exact matrixCoefficient_mem_matrixCoefficientSubalgebraWithAntipode (H := H)
    (b.coord i) (b j)
...
theorem antipode_coefficientMatrix_mem_matrixCoefficientSubalgebraWithAntipode
    ...
    HopfAlgebra.antipode R (coefficientMatrix (C := H) b i j) ∈
      matrixCoefficientSubalgebraWithAntipode (R := R) (H := H) (M := M) := by
  rw [coefficientMatrix_apply]
  exact antipode_matrixCoefficient_mem_matrixCoefficientSubalgebraWithAntipode (H := H)
    (b.coord i) (b j)
...
/-- The algebra generated by the coefficients and their antipode images is the coefficient
algebra of the product of a finite projective comodule with its dual. -/
@[simp]
theorem matrixCoefficientSubalgebraWithAntipode_eq_prod_dual :
    matrixCoefficientSubalgebraWithAntipode (R := R) (H := H) (M := M) =
      matrixCoefficientSubalgebra (R := R) (C := H) (M := M × Module.Dual R M) := by
  rw [matrixCoefficientSubalgebraWithAntipode, matrixCoefficientSubalgebra_prod_dual]
