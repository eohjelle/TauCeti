...
For a finite projective comodule,
`matrixCoefficientSubalgebraWithAntipode_eq_prod_dual` identifies this intrinsically defined
algebra with the coefficient algebra of `M × Mᵛ`. -/
...
theorem matrixCoefficientSubalgebraWithAntipode_def :
    matrixCoefficientSubalgebraWithAntipode (R := R) (H := H) (M := M) =
      matrixCoefficientSubalgebra (R := R) (C := H) (M := M) ⊔
        (matrixCoefficientSubalgebra (R := R) (C := H) (M := M)).map
          (HopfAlgebra.antipodeAlgHom R H) := by
  rw [matrixCoefficientSubalgebraWithAntipode]
    ...
    matrixCoefficientSubalgebraWithAntipode (R := R) (H := H) (M := M) =
      Algebra.adjoin R
        (matrixCoefficientSet (R := R) (C := H) (M := M) ∪
          HopfAlgebra.antipode R '' matrixCoefficientSet (R := R) (C := H) (M := M)) := by
  rw [matrixCoefficientSubalgebraWithAntipode_def, matrixCoefficientSubalgebra_def,
    AlgHom.map_adjoin, Algebra.adjoin_union]
  congr 2
...
theorem matrixCoefficientSubalgebraWithAntipode_eq_prod_dual :
  ...
  rw [matrixCoefficientSubalgebraWithAntipode_def, matrixCoefficientSubalgebra_prod_dual]
