  ...
  · rw [← Algebra.map_top,
    ...
    · rw [BialgHom.coe_toAlgHom, BialgHomClass.map_antipode, coordinateBialgHom_X]
      exact antipode_coefficientMatrix_mem_matrixCoefficientSubalgebraWithAntipode
        (H := H) b i j
  · have hcoeff : matrixCoefficientSubalgebra (R := R) (C := H) (M := M) ≤
    ...
    apply matrixCoefficientSubalgebraWithAntipode_le_iff.mpr
    exact ⟨fun φ m ↦ hcoeff (matrixCoefficient_mem_subalgebra (R := R) (C := H) φ m),
      fun φ m ↦ hanti ⟨matrixCoefficient (R := R) (C := H) φ m,
